...
# # Max IDs per request. The server reads `region` from $_GET (query string) and
# # `trackIDs` from $_POST (form body), so only trackIDs contribute to body size.
# CHUNK_SIZE = 500


# def _request_chunk(region: str, track_ids_str: str) -> pd.DataFrame:
#     """
#     Send one request for a batch of track IDs.

#     The PHP getVariable() helper reads parameters from $_REQUEST, which in most
#     configurations merges $_GET + $_POST. To be safe we put `region` in the
#     query string (guaranteed $_GET) and `trackIDs` in the POST body (avoids the
#     414 URI Too Long that occurs when both go in the URL).
#     """
#     try:
#         r = requests.post(
#             ENDPOINT,
#             params={"region": region},   # goes into query string → $_GET
#             data={"trackIDs": track_ids_str},  # goes into POST body → $_POST
#             timeout=300,
#         )
#         r.raise_for_status()
...
#         df = _request_chunk(region, ",".join(map(str, chunk)))
#         if not df.empty:
#             frames.append(df)
...
#         type=int,
#         default=CHUNK_SIZE,
#         help=f"Max track IDs per request (default: {CHUNK_SIZE})",
#     )
